    ...
    if (!file_exists($_FILES['foto']['tmp_name']) || !is_uploaded_file($_FILES['foto']['tmp_name'])) {
        ...
        if (!in_array($akhiran, $ekstensi)) {
            ...
            if ($ukuran < 1010000020) {
                ...
                move_uploaded_file($_FILES['foto']['tmp_name'], 'siswa/gambar/'.$nmfile);
                ...
                $sql = "INSERT INTO siswa(nis,nama,jk,tempat_lahir, tanggal_lahir,kelas_id,foto) values('$nis','$nama','$jk','$tempat_lahir','$tanggal_lahir', '$kelas_id','$nmfile')";
        } else {
            header("location: index.php?m=siswa");
            echo '<script language="JavaScript">';
